...
function Panel() {
  ...
  // --- MESAJI ELLE DÜZENLEME ---
  // Eğer s.customText varsa, otomatik şablon yerine onu kullanacağız.
  const buildText = (s) => {
  ...
  const previews = useMemo(()=>{
    ...
    return activeGroup.students.filter(s=>!s.optedOut).map(s=>({
      id:s.id, name:s.name, text: buildText(s)
    }))
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[activeGroupId, activeGroup, defaults, mode])

  // --- GRUP EKLE / SİL / YENİDEN ADLANDIR ---
  const [newGroupName, setNewGroupName] = useState('')
  const slugify = (str) => (str || '').toString().trim().toLowerCase()
    .replace(/\s+/g,'-').replace(/[^a-z0-9\-ğüşöçı]/g,'')
  const addGroup = () => {
    ...
    return (
      <div className="app" style={{marginTop:16}}>
        <div className="card">
          ...
          <div className="label" style={{marginBottom:8}}>Yeni Grup Adı</div>
          ...
          <div style={{color:'var(--muted)'}}>Henüz grup yok. Yukarıdan yeni grup ekleyin.</div>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="app" style={{marginTop:16}}>
      ...
      <div className="card" style={{marginBottom:12}}>
        ...
        <div className="grid grid-3">
          ...
          <div style={{display:'flex', gap:8, alignItems:'flex-end'}}>
            ...
            <button className="btn" onClick={deleteGroup} style={{borderColor:'#5b1f1f', background:'#190f0f', color:'#ffb4b4'}}>Grubu Sil</button>
          </div>
        </div>
      </div>
      ...
      {/* Öğrenciler + Kişiye özel düzenleme */}
      <div className="card" style={{marginBottom:12}}>
        ...
        <div className="grid">
          {(activeGroup?.students||[]).map(s=>(
            <div key={s.id} className="student">
              ...
              {/* Önizleme + Elle düzenleme */}
              <div style={{marginTop:10}}>
                <div className="label">Önizleme</div>
                <div className="preview">{buildText(s)}</div>

                <div style={{marginTop:8}}>
                  <div className="label">Kişiye Özel Düzenle (opsiyonel)</div>
                  <textarea className="textarea" placeholder="Bu öğrenci için mesajı elle değiştir (boş bırakılırsa otomatik şablon kullanılır)"
                    value={s.customText||''}
                    onChange={e=>patchStudent(s.id,{customText:e.target.value})}/>
      ...
      <div className="footer-note">Not: WhatsApp gönderimi için backend /send eklenecek.</div>
    </div>
  )
}
