...
function Panel(){
  ...
  const [defaults,setDefaults]=useState({ day:"Cuma", time:"15:00", place:"Öğrenci evinde", topic:"Namazın önemi" }); // sadece pre

  /* formlar */
  const [newGroupName,setNewGroupName]=useState("");
  const [newStudent,setNewStudent]=useState({ name:"", parent_phone:"" }); // sınıf kaldırıldı

  /* -------- Supabase: grupları + öğrencileri çek -------- */
  const fetchAll = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(`
        ...
        students (
          ...
          day, time, place, topic, homework, custom_text, opted_out, created_at
        )
      `)
    ...
    if (error) {
      ...
      const rows=(data||[]).map(g=>({ ...g, students: g.students || [] }));
      setGroups(rows);
      if (!activeGroupId && rows[0]) setActiveGroupId(rows[0].id);
    }
    setLoading(false);
  };
  ...
  const previews = useMemo(()=>{
    ...
    return (activeGroup.students||[])
      ...
      .map(s=>({ id:s.id, name:s.name, text:buildText(s) }));
            ...
            {groups.map(g=>(
              <div key={g.id} className="student" style={{display:"flex", justifyContent:"space-between", alignItems:"center"}}>
                <div>
                  ...
                  <div style={{color:"var(--muted)", fontSize:12}}>{(g.students||[]).length} öğrenci</div>
                </div>
                <div style={{display:"flex", gap:8}}>
                  <button className="btn btn-primary" onClick={()=>{ setActiveGroupId(g.id); setScreen("groupDetail"); }}>Aç</button>
              ...
              {mode==="pre" ? (
                <div className="grid grid-3" style={{marginTop:8}}>
                  ...
                  <div style={{gridColumn:"1 / -1"}}>
                    ...
                    <input className="input" value={s.topic||""} onChange={e=>patchStudent(s.id,{topic:e.target.value})} placeholder="(boş=varsayılan)" />
                  </div>
                </div>
              ) : (
